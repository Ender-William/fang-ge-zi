...
class FamilyRepositoryImpl @Inject constructor(
    ...
    private suspend fun buildLineage(pigeonId: String, generation: Int, maxGeneration: Int): LineageResult {
        ...
        val relation = familyRelationDao.getByPigeonId(pigeonId)
        val father = relation?.fatherId?.let { buildLineage(it, generation + 1, maxGeneration) }
        val mother = relation?.motherId?.let { buildLineage(it, generation + 1, maxGeneration) }
